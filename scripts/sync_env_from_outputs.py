...
def latest_outputs_with_values(resource_group: str) -> Dict[str, Any]:
    raw = run_az(["az", "deployment", "group", "list", "-g", resource_group, "-o", "json"])
    deployments = json.loads(raw)
    if not deployments:
        raise RuntimeError(f"No deployments found in resource group: {resource_group}")
    # sort newest → oldest
    deployments.sort(key=lambda d: d.get("properties", {}).get("timestamp", ""), reverse=True)
    for d in deployments:
        outputs = (d.get("properties") or {}).get("outputs") or {}
        # non-empty dict with at least one .value
        if isinstance(outputs, dict) and any(isinstance(v, dict) and "value" in v for v in outputs.values()):
            return outputs
    # none had outputs
    return {}
...
def main() -> int:
    ...
    ap.add_argument("--rg", "--resource-group", dest="rg", required=True, help="Resource group name")
    ap.add_argument("--env-name", required=True, help="AZURE_ENV_NAME (e.g., dev)")
    args = ap.parse_args()

    outputs = latest_outputs_with_values(args.rg)
    if not outputs:
        print(f"No deployments with non-empty outputs found in {args.rg}.", file=sys.stderr)
        return 2
