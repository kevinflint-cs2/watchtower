...
def main():
    ...
    for c in conns:
        ...
        if "bing" in text_blob and ("grounding" in text_blob or "bing.grounding" in text_blob):
            bing_candidates.append((name, cid, kind or ctype))

    if not bing_candidates:
        print("No Bing Grounding connection found. Check the Project > Connected resources in the portal.")
        print("Connections discovered:")
        for c in conns:
            print(f" - {getattr(c,'name','<no-name>')} | kind={getattr(c,'kind','')} | type={getattr(c,'type','')} | id={getattr(c,'id','')}")
        return
